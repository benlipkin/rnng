...
def main(args):
    ...
    if args.test == 1:
        ...
        test_ppl = eval(test_data, model, args, count_eos_ppl=args.count_eos_ppl)
        sys.exit(0)
    best_val_ppl = eval(val_data, model, args, count_eos_ppl=args.count_eos_ppl)
    while epoch < args.num_epochs:
        ...
        for i in np.random.permutation(len(train_data)):
            ...
            if length == 1:
                continue
            sents = sents.cuda() if args.gpu != -1 else sents
            b += 1
            optimizer.zero_grad()
            optimizer.zero_grad()
            ...
            if b % args.print_every == 0:
                ...
                print(
                    "Epoch: %d, Batch: %d/%d, LR: %.4f, TrainPPL: %.2f, |Param|: %.4f, BestValPerf: %.2f, Throughput: %.2f examples/sec"
                    % (
                        epoch,
                        b,
                        len(train_data),
                        args.lr,
                        np.exp(train_nll / num_words),
                        param_norm,
                        best_val_ppl,
        ...
        val_ppl = eval(val_data, model, args, count_eos_ppl=args.count_eos_ppl)
        print("--------------------------------")
...
def eval(data, model, args, count_eos_ppl=0):
    model.eval()
    num_words = 0
    total_nll = 0.0
    with torch.no_grad():
        for i in list(reversed(range(len(data)))):
            ...
            if (
                length == 1
            ):  # we ignore length 1 sents in URNNG eval so do this for LM too
                continue
