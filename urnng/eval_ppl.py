...
parser.add_argument(
    "--is_temp", default=2.0, type=float, help="divide scores by is_temp before CRF"
)
parser.add_argument(
    "--samples", default=1000, type=int, help="samples for IS calculation"
)
parser.add_argument(
...
def main(args):
    ...
    with torch.no_grad():
        for i in list(reversed(range(len(data)))):
            ...
            print(
                "Batch: %d/%d, ElboPPL: %.2f, KL: %.4f, IwaePPL: %.2f"
                % (
                    i,
                    len(data),
                    np.exp((total_nll_recon + total_kl) / num_words),
                    total_kl / num_sents,
                    np.exp(total_nll_iwae / num_words),
                )
            )
    ...
    print(
        "ElboPPL: %.2f, ReconPPL: %.2f, KL: %.4f, IwaePPL: %.2f"
