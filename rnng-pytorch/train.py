...
parser.add_argument(
    "--save_path", default="rnng.pt", help="where to save the best model"
)
parser.add_argument(
...
def main(args):
    ...
    val_data = Dataset.from_json(
        args.val_file, args.batch_size, vocab, action_dict, oracle=args.strategy
    )
    ...
    while epoch <= args.num_epochs and not is_early_stop():
        ...
        def batch_step(
            ...
            for begin_idx in range(0, token_ids.size(0), block_size):
                ...
                div_subword_end_mask = subword_end_mask[
                    begin_idx : begin_idx + block_size
                ]
...
def do_valid(
    model, optimizer, scheduler, train_data, val_data, tb, epoch, step, val_losses, args
):
