...
class FixedInOrderStack(FixedStack):
    def __init__(self, initial_hidden, stack_size, input_size, beam_size=1):
        super(FixedInOrderStack, self).__init__(
            initial_hidden, stack_size, input_size, beam_size
        )

        # An extra storage necessary to achieve correct in-order implementation for subwords.
        self.last_subword_begin_idx = self.nt_index_pos.new_zeros(
            self.nt_index_pos.size()
        )
    ...
    def do_shift(self, shift_batches, shifted_embs, subword_end_mask):
        ...
        begin_subword_shift_batches = tuple(
            [item[is_begin_subword] for item in shift_batches]
        )
        self.last_subword_begin_idx[begin_subword_shift_batches] = self.top_position[
            begin_subword_shift_batches
        ]
    ...
    def do_nt(self, nt_batches, nt_embs, nt_ids):
        ...
        span_idx = (subword_begin_idx.unsqueeze(-1) + span_idx_order).view(-1)[
            span_idx_mask
        ] - 1
        ...
        for nt_batch in nt_batches:
            nt_batches_for_move += (
                nt_batch.unsqueeze(-1)
                .expand(-1, max_span_length)
                .reshape(-1)[span_idx_mask],
            )
        nt_batches_for_move += (
            span_idx,
        )  # reason why not +1 here is due to offset of self.trees relative to self.hiddens.
        nt_batches_for_move_tgt = nt_batches_for_move[:-1] + (
            nt_batches_for_move[-1] + 1,
        )
...
class InOrderStackState(StackState):
    ...
    def update_nt_counts(self, actions, action_dict, action_path):
        ...
        reset_ncons_reduce_mask = (actions == action_dict.a2i["REDUCE"]) * (
            prev_is_not_nt
        )
...
class FixedStackInOrderRNNG(FixedStackRNNG):
    ...
    def new_beam_stack_with_state(self, initial_hidden, stack_size, beam_size):
        stack = FixedInOrderStack(
            initial_hidden, stack_size, self.input_size, beam_size
        )
    ...
    def invalid_action_mask(self, beam, sent_lengths, subword_end_mask):
        ...
        prev_is_subword_end_mask = (beam.stack.pointer > 0) * subword_end_mask.gather(
            1, prev_pointer
        )
        ...
        finish_mask = (
            (pointer != sent_lengths) + (nopen_parens != 0) + (top_position != 1)
        ).unsqueeze(-1) * finish_mask
        ...
        finished_mask = (
            (prev_actions == self.action_dict.finish_action())
            + (prev_actions == self.action_dict.padding_idx)
        ).unsqueeze(-1)
        beam_width_mask = (
