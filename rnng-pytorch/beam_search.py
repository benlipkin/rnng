...
parser.add_argument("--rep_file", default="reps.txt")
    ...
    def try_parse(tokens, subword_end_mask, stack_size_bound=None):
        ...
        if args.dump_beam:
            parses, surprisals, beam_history = parse(
                tokens, subword_end_mask, True, stack_size_bound
            )
        ...
        for batch in batches:
            ...
            if any(len(p) == 0 for p in parses):
                ...
                if args.stack_size_bound < tokens.size(1):
                    ...
                    parses, surprisals, beam_history = try_parse(
            ...
            ############################
            actions = torch.Tensor(best_actions).to(torch.int64).to(device)
            reps = model.get_reps(tokens, actions, subword_end_mask)
            reps_json = json.dumps(reps)
            with open(args.rep_file, "w") as f:
                f.write(reps_json)
            ############################
